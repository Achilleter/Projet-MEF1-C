...
void berserk(Champ* xavier, Equipe* allie){
    ...
    for(int i=0; i<3; i++){ // verifie si la technique combinee est possible
        if(strcmp(allie->membres[i].tech.nom, "cadeau_empoisonne")==0){
            ...
            if(allie->boolia==0){
                if(allie->membres[i].jaugeactuelle==allie->membres[i].jaugemax && allie->membres[i].statut==1){
                    // Demande a l'utilisateur s'il veut utiliser la technique combinee
    ...
    //technique de xavier
    // augmente l'attaque de 20 pendant 1 tour et ce soigne de 50, applique l'invincibilite pendant 1 tour puis s'immobilise pendant 1 tour
    if(xavier->tech.nbtactifs == 0){
        ...
        appeffetStatut(xavier,4,2);//applique invincibilite pendant 1 tour
    }
    else if(xavier->tech.nbtactifs != 0){// Diminution des tours de la technique si elle est deja active
...
void flashbacks(Champ* nathalie, Champ* allie1, Champ* allie2){
    ...
    //soigne de 60 tous les allies et leur enlève tous les effets statuts negatifs (sauf les effets comme l'invincibilite, le renvoie de degats et la provocation)
    nathalie->jaugeactuelle = 0;
    printf("Nathalie utilise Flashbacks !\n");
    appeffetStat(allie1,4,60);//soigne de 6O tous les allies
    appeffetStat(allie2,4,60);
    appeffetStat(nathalie,4,60);
    int booltemp=0;
    // enlève tous les effets statuts negatifs pour chaque allie (sauf les effets comme l'invincibilite, le renvoie de degats et la provocation)
    for(int i=0; i<allie1->nbeffets; i++){
        if(booltemp==1){// Verification si un effet a ete supprime alors decrementer le compteur
            i--;
        }
        booltemp=0;
...
void muraille(Champ* smasheur){
    ...
    //provoque les ennemis, augmente sa defense de 30 et renvoie les degats pendant 1 tour
    if(smasheur->tech.nbtactifs == 0){
        smasheur->tech.nbtactifs = 2;
        printf("Smasheur utilise muraille!\n");
        smasheur->jaugeactuelle = 0;
        appeffetStatut(smasheur,5,2);//applique l'effet renvoie de degats pendant 1 tour
        appeffetStatut(smasheur,3,2);//applique l'effet provocation pendant 1 tour
        appeffetStat(smasheur,2,30);//augmente la defense de 30
    }
    else if(smasheur->tech.nbtactifs != 0){ // Diminution des tours de la technique si elle est deja active
...
void cadeau_empoisonne(Champ* steve, Equipe* ennemi, Equipe* allie){
    ...
    for(int i=0; i<3; i++){
        if(strcmp(allie->membres[i].tech.nom, "berserk")==0){ // verifie si la technique combinee est possible
            int choix=0;
            if(allie->boolia==0){
                if(allie->membres[i].jaugeactuelle==allie->membres[i].jaugemax && allie->membres[i].statut==1){
                    // Demande a l'utilisateur s'il veut utiliser la technique combinee
...
void fossoyeur_des_mondes(Champ* booga){
    ...
    if(booga->tech.nbtactifs == 0){
        ...
        appeffetStat(booga, 2, 10); // Augmente la defense de 10
    }
    else if(booga->tech.nbtactifs == 1){
        appeffetStat(booga, 2, -10); // Diminue la defense de 10
...
void cryogenese(Champ* sandrine, Equipe* ennemi){
    ...
    // inflige 30 de degats a tous les ennemis et stun pendant 1 tour
...
void scierculaire(Champ* annesophie, Equipe* ennemi, Equipe* allie){
    ...
    for(int i=0; i<3; i++){
        if(strcmp(allie->membres[i].tech.nom, "bourreau")==0){ // verifie si la technique combinee est possible
            int choix=0;
            if(allie->boolia==0){
                if(allie->membres[i].jaugeactuelle==allie->membres[i].jaugemax && allie->membres[i].statut==1){
                    // Demande a l'utilisateur s'il veut utiliser la technique combinee
                    printf("\nVoulez vous utiliser la technique combinee avec %s (1:oui, 0:non)\n", allie->membres[i].nom);
                    int verif;
                    do{
                        verif=scanf("%d", &choix);
                        if (choix<0 || choix>1) {
    ...
    //inflige 50 degats a tous les ennemis et applique l'effet poison pendant 1 tour
...
void cicatrices_eternels(Champ* gaby, Equipe* ennemi, Equipe* allie){
    ...
    for(int i=0; i<3; i++){
        if(strcmp(allie->membres[i].tech.nom, "reinitialisation")==0){ // verifie si la technique combinee est possible
            int choix=0;
            if(allie->boolia==0){
                if(allie->membres[i].jaugeactuelle==allie->membres[i].jaugemax && allie->membres[i].statut==1){
                    // Demande a l'utilisateur s'il veut utiliser la technique combinee
    ...
    //technique de gaby
    // inflige 125 de degats a tous les ennemis
...
void reinitialisation(Champ* clara, Champ* allie1, Champ* allie2, Equipe* allies, Equipe* ennemi){
    ...
    int verif;
    for(int i=0; i<3; i++){
        if(strcmp(allies->membres[i].tech.nom, "cicatrices_eternels")==0){ // verifie si la technique combinee est possible
            int choix=0;
            if(allies->boolia==0){
    ...
    //technique de clara
    // Ressucite un allie avec la moitie de ses points de vies
    verif=0;
    if (allie1->statut==1 && allie2->statut==1) { // Si les deux allies sont vivants
        printf("Aucun de vos allies n'est mort !\n");
        return;
    }
    else if (allie1->statut==0 && allie2->statut==0) { // Si les deux allies sont morts
        choix=0;
        printf("Choississez un allie a ressusciter (1=%s, 2=%s) :\n", allie1->nom, allie2->nom);
        do {
            verif=scanf("%d", &choix);
            if (choix<1 || choix>2) {
                printf("Index invalide\n");
    ...
    else { // Si un seul allie est mort
        printf("Clara utilise Reinitialisation !\n");
        clara->jaugeactuelle = 0;
        if (allie1->statut==0) {
            printf("Vous ressuscitez %s !\n", allie1->nom);
...
void extermination(Champ* annesophie, Champ* zed, Equipe* adversaires){
    ...
    // Les personnages combinent leurs techniques pour infliger 60 de degats a tous les ennemis et applique l'effet bourreau pendant 1 tour
    ...
    for(int i=0; i<3; i++){
        if(adversaires->membres[i].statut==1){
            ...
            appeffetStatut(&adversaires->membres[i],6,1); //applique l'effet bourreau a tous les ennemis pendant 1 tour
        }
    }
}
...
void sacrifice_fraternel(Champ* xavier, Champ* steve){
    ...
    // Steve donne la moitie de ses stats a Xavier
    ...
    appeffetStat(xavier, 3, (steve->vitesse)/2);
...
void retour_a_la_haine(Champ* gaby, Champ* clara, Equipe* adversaires, Equipe* allie){
    ...
    printf("Gaby attire Clara dans les tenebres pour l'aider a supprimer un ennemi!\n");
    gaby->jaugeactuelle = 0;
    clara->jaugeactuelle = 0;
    int choix=0;
    int verif;
